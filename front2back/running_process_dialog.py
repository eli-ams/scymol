...
import front2back.fron2back_static_functions
from logging_functions import print_to_log, log_function_call

Ui_ProcessDialog, _ = uic.loadUiType("frontend/uis/process_dialog.ui")
...
class RunningProcessDialog(QDialog):
    ...
    def update_mixture_progress_bar(self) -> None:
        ...
        self.current_mixture = int(
            (
                front2back.fron2back_static_functions.get_latest_mixture_folder_by_count(
    ...
    def update_lammps_stage_progress_bar(self) -> None:
        ...
        try:
            (
                total_progress,
                total_substages,
            ) = front2back.fron2back_static_functions.calculate_progress(
                directory=f"output/{self.job_id}/mixture_{self.current_mixture}",
    ...
    def read_log_file(self) -> None:
        ...
        try:
            with open(f"output/{self.job_id}/log.txt", "r") as f:
                content = f.read()
    ...
    def explore_output_folder(self) -> None:
        ...
        current_job_directory = os.path.join(os.getcwd(), f"output/{self.job_id}")
